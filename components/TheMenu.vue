...
<template>
  <Transition @leave="onMenuButtonLeave">
    <button v-if="!navIsOpen" class="fixed bottom-4 left-[8.3vw] rounded-full bg-adura-purple grid grid-rows-[max-content_max-content] justify-center content-center w-[90px] lg:w-[130px] h-[90px] lg:h-[130px] font-semibold text-adura-black text-sm lg:text-base" @click="showNav">
  ...
  <Transition @enter="onNavEnter">
    <nav v-if="navIsOpen" ref="nav" class="fixed bg-adura-black border border-black flex items-center justify-between w-[95.6%] left-[2.2%] bottom-4 rounded-[32px] px-20 py-10">
      <a class="bg-white flex rounded-3xl px-8 py-4 items-center gap-x-2" href="mailto:abiolaaduragbemiaa@gmail.com">
        <p class="font-normal text-2xl leading-[30px] text-adura-black">Get In Touch</p>
      ...
      <ul>
        <li class="flex gap-x-10 font-light text-white text-2xl leading-[30px]">
          <NuxtLink to="/">Home</NuxtLink>
          <NuxtLink to="/work">Work</NuxtLink>
          <NuxtLink to="/shots">Shots</NuxtLink>
          <NuxtLink to="/about">About</NuxtLink>
          <NuxtLink to="/process">My Process</NuxtLink>
        </li>
      </ul>
      <ul>
        <li class="flex gap-x-4">
          <a class="bg-white flex justify-center items-center rounded-full w-12 h-12" href="" target="_blank">
